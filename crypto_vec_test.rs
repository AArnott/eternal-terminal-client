use xsalsa20poly1305::aead::{Aead, KeyInit};
use xsalsa20poly1305::{Key, Nonce, XSalsa20Poly1305};

fn main() {
    // TweetNaCl / NaCl box test - crypto_secretbox with 32-byte zero pad
    // From nacl-20110221/tests/secretbox.c expected c:
    let key = [
        0x1b,0x27,0x55,0x64,0x73,0xe9,0x85,0xd4,0x62,0xcd,0x51,0x19,0x7a,0x9a,0x46,0xc7,
        0x60,0x09,0x54,0x9e,0xac,0x64,0x74,0xf2,0x06,0xc4,0xee,0x08,0x44,0xf6,0x83,0x89
    ];
    let nonce = [
        0x69,0x69,0x6e,0xe9,0x55,0xb6,0x2b,0x73,0xcd,0x62,0xbd,0xa8,0x75,0xfc,0x73,0xd6,
        0x82,0x19,0xe0,0x03,0x6b,0x7a,0x0b,0x37
    ];
    // plaintext without the 32-byte zero pad (mlen after zeros)
    let m = [
        
    ];
    // Expected c from nacl (first 16 are zeros in classic API, bytes 16.. are mac||ct for easy)
    // From https://github.com/jedisct1/libsodium/blob/master/test/default/secretbox.c
    let expected_easy = [
        
    ];

    let cipher = XSalsa20Poly1305::new(Key::from_slice(&key));
    let n = Nonce::from_slice(&nonce);
    let ct = cipher.encrypt(n, m.as_slice()).unwrap();
    println!("ct len: {}", ct.len());
    println!("expected len: {}", expected_easy.len());
    
    // RustCrypto: ciphertext || tag
    let split = ct.len() - 16;
    let mut mac_first = Vec::new();
    mac_first.extend_from_slice(&ct[split..]);
    mac_first.extend_from_slice(&ct[..split]);
    
    println!("mac_first match: {}", mac_first == expected_easy);
    println!("tag_last match:  {}", ct.as_slice() == expected_easy.as_slice());
    
    // Print first 32 of each
    print!("got mac_first: ");
    for b in mac_first.iter().take(32) { print!("{:02x}", b); }
    println!();
    print!("expected:      ");
    for b in expected_easy.iter().take(32) { print!("{:02x}", b); }
    println!();
    print!("got tag_last:  ");
    for b in ct.iter().take(32) { print!("{:02x}", b); }
    println!();
}
