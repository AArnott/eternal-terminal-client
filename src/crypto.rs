//! libsodium-compatible `crypto_secretbox` (XSalsa20-Poly1305).
//!
//! Eternal Terminal uses libsodium's `crypto_secretbox_easy`. The RustCrypto
//! `xsalsa20poly1305` crate already uses NaCl wire order (**MAC prepended**),
//! so we pass ciphertext through unchanged.

use parking_lot::Mutex;
use thiserror::Error;
use xsalsa20poly1305::aead::{Aead, KeyInit};
use xsalsa20poly1305::{Key, Nonce, XSalsa20Poly1305};

pub const KEY_BYTES: usize = 32;
pub const NONCE_BYTES: usize = 24;
pub const MAC_BYTES: usize = 16;

/// Client → server encrypt stream (matches C++ `CLIENT_SERVER_NONCE_MSB`).
pub const CLIENT_SERVER_NONCE_MSB: u8 = 0;
/// Server → client decrypt stream (matches C++ `SERVER_CLIENT_NONCE_MSB`).
pub const SERVER_CLIENT_NONCE_MSB: u8 = 1;

#[derive(Debug, Error)]
pub enum CryptoError {
    #[error("invalid key length: expected {KEY_BYTES}, got {0}")]
    InvalidKeyLength(usize),
    #[error("encryption failed")]
    Encrypt,
    #[error("decryption failed (possible key mismatch)")]
    Decrypt,
    #[error("ciphertext too short")]
    Truncated,
}

pub struct CryptoHandler {
    inner: Mutex<Inner>,
}

struct Inner {
    cipher: XSalsa20Poly1305,
    nonce: [u8; NONCE_BYTES],
}

impl CryptoHandler {
    /// Create a handler from the 32-byte session passkey and stream MSB.
    pub fn new(key: &[u8], nonce_msb: u8) -> Result<Self, CryptoError> {
        if key.len() != KEY_BYTES {
            return Err(CryptoError::InvalidKeyLength(key.len()));
        }
        let mut nonce = [0u8; NONCE_BYTES];
        nonce[NONCE_BYTES - 1] = nonce_msb;

        let cipher = XSalsa20Poly1305::new(Key::from_slice(key));
        Ok(Self {
            inner: Mutex::new(Inner { cipher, nonce }),
        })
    }

    pub fn encrypt(&self, plaintext: &[u8]) -> Result<Vec<u8>, CryptoError> {
        let mut g = self.inner.lock();
        g.increment_nonce();
        let nonce = Nonce::from_slice(&g.nonce);
        // xsalsa20poly1305 uses NaCl/libsodium order: MAC || ciphertext
        g.cipher
            .encrypt(nonce, plaintext)
            .map_err(|_| CryptoError::Encrypt)
    }

    pub fn decrypt(&self, data: &[u8]) -> Result<Vec<u8>, CryptoError> {
        if data.len() < MAC_BYTES {
            return Err(CryptoError::Truncated);
        }
        let mut g = self.inner.lock();
        g.increment_nonce();
        let nonce = Nonce::from_slice(&g.nonce);
        g.cipher
            .decrypt(nonce, data)
            .map_err(|_| CryptoError::Decrypt)
    }
}

impl Inner {
    /// Matches libsodium/ET: increment nonce little-endian from index 0.
    fn increment_nonce(&mut self) {
        for b in &mut self.nonce {
            *b = b.wrapping_add(1);
            if *b != 0 {
                break;
            }
        }
    }
}

#[cfg(test)]
mod tests {
    use super::*;

    #[test]
    fn encrypt_decrypt_roundtrip() {
        let key = b"0123456789abcdef0123456789abcdef";
        let enc = CryptoHandler::new(key, CLIENT_SERVER_NONCE_MSB).unwrap();
        let dec = CryptoHandler::new(key, CLIENT_SERVER_NONCE_MSB).unwrap();
        let ct = enc.encrypt(b"hello et").unwrap();
        assert_eq!(dec.decrypt(&ct).unwrap(), b"hello et");
        // Non-empty: ciphertext must be longer than MAC alone
        assert!(ct.len() > MAC_BYTES);
    }

    #[test]
    fn empty_message_roundtrip() {
        let key = b"0123456789abcdef0123456789abcdef";
        let enc = CryptoHandler::new(key, CLIENT_SERVER_NONCE_MSB).unwrap();
        let dec = CryptoHandler::new(key, CLIENT_SERVER_NONCE_MSB).unwrap();
        let ct = enc.encrypt(b"").unwrap();
        assert_eq!(ct.len(), MAC_BYTES);
        assert_eq!(dec.decrypt(&ct).unwrap(), b"");
    }

    #[test]
    fn independent_streams() {
        let key = b"0123456789abcdef0123456789abcdef";
        let c2s = CryptoHandler::new(key, CLIENT_SERVER_NONCE_MSB).unwrap();
        let s2c = CryptoHandler::new(key, SERVER_CLIENT_NONCE_MSB).unwrap();
        let a = c2s.encrypt(b"a").unwrap();
        let b = s2c.encrypt(b"b").unwrap();
        assert_ne!(a, b);
    }

    /// NaCl/libsodium `crypto_secretbox` vector (xsalsa20poly1305 crate tests).
    #[test]
    fn nacl_secretbox_test_vector() {
        let key = [
            0x1b, 0x27, 0x55, 0x64, 0x73, 0xe9, 0x85, 0xd4, 0x62, 0xcd, 0x51, 0x19, 0x7a, 0x9a,
            0x46, 0xc7, 0x60, 0x09, 0x54, 0x9e, 0xac, 0x64, 0x74, 0xf2, 0x06, 0xc4, 0xee, 0x08,
            0x44, 0xf6, 0x83, 0x89,
        ];
        let nonce = [
            0x69, 0x69, 0x6e, 0xe9, 0x55, 0xb6, 0x2b, 0x73, 0xcd, 0x62, 0xbd, 0xa8, 0x75, 0xfc,
            0x73, 0xd6, 0x82, 0x19, 0xe0, 0x03, 0x6b, 0x7a, 0x0b, 0x37,
        ];
        let plaintext = [
            ,
        ];
        let expected: [u8; 147] = [
            ,
        ];

        let cipher = XSalsa20Poly1305::new(Key::from_slice(&key));
        let n = Nonce::from_slice(&nonce);
        let ct = cipher.encrypt(n, plaintext.as_slice()).unwrap();
        assert_eq!(ct.as_slice(), &expected[..], "must match NaCl secretbox");

        // CryptoHandler uses a counter nonce starting from MSB; test raw cipher above.
        // Also verify open:
        let pt = cipher
            .decrypt(Nonce::from_slice(&nonce), expected.as_slice())
            .unwrap();
        assert_eq!(pt.as_slice(), &plaintext[..]);
    }
}
